...
def label_components(img):
    ...
    num_labels = 1
    equiv_classes = []
    for row in range(0, h - 1):   # traverse image rows for pixels
        for col in range(0, w - 1):  # traverse image columns foe pixels 
            if label_img[row, col] != 0:  # if pixel is part of a component
                upper = label_img[row-1, col]  # pixel to above of the pixel in consideration 
                left = label_img[row, col-1]  # pixel to the left of the pixel in consideration
                if left == 0 and upper == 0:  # if neighbor (left and upper) are of background
                    label_img[row, col] = num_labels
                    num_labels = num_labels + 1
                elif left == upper: 
                    label_img[row, col] = left
                elif upper == 0 and left != 0:  # if pixel to above the pixel in consideration is from background
                    label_img[row, col] = left
                elif left == 0 and upper != 0:  # if pixel to the left of the pixel in consideration is from background
                    label_img[row, col] = upper
	        elif left != upper:
                    label_img[row, col] = min(left, upper)
                # Use the following code whenever you find an object pixel with mismatching upper and left
                    need_new_class= True
